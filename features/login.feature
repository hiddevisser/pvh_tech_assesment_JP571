...
Feature: Testing the login functionality of the Tommy site
  ...
  Scenario Outline: As a user, I should see an error message when entering invalid credentials
    ...
    When I login with <username> and and <password>
    Then I should see an error message saying <message>
    Examples:
      | username             | password             | message                                                             |
      | hidde_test@gmail.com | SuperSecretPassword! | Je gebruikersnaam en wachtwoord matchen niet. Probeer het nog eens. |
      | hidde_test           | SuperSecretPassword! | Sorry, dit is geen geldig e-mailadres                               |
